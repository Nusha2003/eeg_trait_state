from physionet_dataloader import make_physionet_ae_dataloaders
from biodl.trainer.trainer import TrainerEEG
from biodl.trainer.trainer import PreTrainerEEG
...
logdir = args.logdir if args.logdir is not None else os.getenv("BIODL_LOGDIR")
...
config_path = os.path.join(os.getenv("BIODL_ROOT"), "config", "finetune", f"{dataset}.json") if args.config is None else args.config
...
train_dataloader, test_dataloader = make_physionet_ae_dataloaders(
    data_root="/scratch1/amadapur/data/physionet/physionet.org/files/eegmmidb/1.0.0",
    batch_size=32,
    sfreq=160,
    epoch_len_sec=4.0,
)
...
for fold, channel_idx, channel_names, train_dataloader, val_dataloader, test_dataloader in dataloaders_fn():
    ...
    pretrainer = PreTrainerEEG(
        **model_info, logdir=logdir_fold, **config["pretrainer_args"]
    )

    pretrainer.train(
        train_dataloader=train_dataloader,
        val_dataloader=test_dataloader,
        **config["train_fn_args"]
    )
